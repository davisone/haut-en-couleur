'use client';

import Link from 'next/link';
import {useTranslations, useLocale} from 'next-intl';

export default function Footer() {
  const currentYear = new Date().getFullYear();
  const t = useTranslations('Footer');
  const locale = useLocale();

  return (
    <footer id="contact" className="bg-gradient-to-br from-gray-800 to-gray-900 text-white">
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {/* Contact Information */}
          <div>
            <h3 className="text-2xl font-bold mb-6">{t('contactUs')}</h3>
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                <svg className="w-6 h-6 flex-shrink-0 mt-1" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/>
                </svg>
                <div>
                  <p className="font-semibold">{t('address')}</p>
                  <p className="text-white/90">{t('addressValue')}</p>
                  <p className="text-white/90">{t('country')}</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <svg className="w-6 h-6 flex-shrink-0 mt-1" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/>
                </svg>
                <div>
                  <p className="font-semibold">{t('phone')}</p>
                  <a href="tel:+33666284458" className="text-white/90 hover:text-white transition-colors">
                    06 66 28 44 58
                  </a>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <svg className="w-6 h-6 flex-shrink-0 mt-1" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/>
                </svg>
                <div>
                  <p className="font-semibold">{t('email')}</p>
                  <a href="mailto:contact@haut-en-couleur.fr" className="text-white/90 hover:text-white transition-colors">
                    contact@haut-en-couleur.fr
                  </a>
                </div>
              </div>
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h3 className="text-2xl font-bold mb-6">{t('quickLinks')}</h3>
            <nav className="flex flex-col gap-3">
              <Link href={`/${locale}/`} className="text-white/90 hover:text-white transition-colors">
                {t('home')}
              </Link>
              <Link href={`/${locale}/qui-sommes-nous`} className="text-white/90 hover:text-white transition-colors">
                {t('about')}
              </Link>
              <Link href={`/${locale}/realisations`} className="text-white/90 hover:text-white transition-colors">
                {t('achievements')}
              </Link>
              <Link href={`/${locale}/prestations`} className="text-white/90 hover:text-white transition-colors">
                {t('services')}
              </Link>
              <Link href={`/${locale}/contact`} className="text-white/90 hover:text-white transition-colors">
                {t('contact')}
              </Link>
            </nav>
          </div>

          {/* Horaires */}
          <div>
            <h3 className="text-2xl font-bold mb-6">{t('hours')}</h3>
            <p className="text-white/90 mb-4">
              {t('hoursValue')}
            </p>
            <p className="text-white/80 text-sm mt-6">
              {t('description')}
            </p>
          </div>
        </div>

        {/* Copyright & Legal Links */}
        <div className="border-t border-white/20 mt-12 pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-white/80 text-center md:text-left">
              © {currentYear} Haut en Couleur. {t('rights')}
            </p>
            <div className="flex flex-wrap justify-center gap-4 md:gap-6">
              <Link
                href={`/${locale}/mentions-legales`}
                className="text-white/80 hover:text-white transition-colors text-sm"
              >
                {t('legal')}
              </Link>
              <Link
                href={`/${locale}/politique-confidentialite`}
                className="text-white/80 hover:text-white transition-colors text-sm"
              >
                {t('privacy')}
              </Link>
              <Link
                href={`/${locale}/cookies`}
                className="text-white/80 hover:text-white transition-colors text-sm"
              >
                {t('cookies')}
              </Link>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}